...
uint32_t	wavetab[NUMOSCILLATORS][WAVETABLE_SIZE];
...
uint8_t	angle;
	...
	for ( i=start;i<end;i++)
	{
		...
		osc_buf[osc_number][i] = (wavetab[osc_number][angle] * Oscillator[osc_number].current_volume ) >> DAC_BIT;
...
uint32_t	i;
	for(i=0;i<WAVETABLE_SIZE;i++)
		wavetab[osc_number][i] = (uint16_t )((HALF_DAC_RESOLUTION + sinf(i*2*M_PI/256.0f))* (float )(HALF_DAC_RESOLUTION-1));
}
...
float	delta_phase;
	...
	Oscillator[osc_number].volume = volume;
